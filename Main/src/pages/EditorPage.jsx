...
import SOPReferencesPanel from '../components/Editor/SOP/SOPReferencesPanel'
...
import VariablesPanel from '../components/Editor/Contract/VariablesPanel'
import WorkflowTimeline from '../components/Editor/Contract/WorkflowTimeline'
import ReviewActions from '../components/Editor/Contract/ReviewActions'
import ReviewLinkPanel from '../components/Editor/Contract/ReviewLinkPanel'
...
const EditorPage = ({ isEmbedded = false, initialDocId = undefined }) => {
  ...
  const manualSave = useCallback(async () => {
    ...
    try {
      if (!documentId) {
        ...
        if (!isEmbedded) localStorage.setItem('current_document_id', createdId)
  ...
  const createNewDocumentHandler = useCallback(() => {
    ...
    setDocumentId(null)
    if (!isEmbedded) localStorage.removeItem('current_document_id')
  ...
  const duplicateAsNewDocumentHandler = useCallback(async () => {
    ...
    try {
      ...
      if (!isEmbedded) localStorage.setItem('current_document_id', newDoc.id)
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        let docId = initialDocId !== undefined
          ? initialDocId
          : localStorage.getItem('current_document_id')

        if (docId) {
          try {
            await refreshDocumentFromBackend(docId)
            setDocumentId(docId)
          } catch (fetchError) {
          ...
          if (saved) {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              ...
              editor.commands.setContent(first.json || { type: 'doc', content: [] }, false)
              setVariables(first.metadata?.variables || {})
            }
          } else {
            setVersions([])
            setCurrentVersionId('v1')
    ...
    init()
  }, [editor, setVariables, initialDocId, isEmbedded])
  ...
  return (
    <div className={`editor-page-container${isEmbedded ? ' editor-embedded' : ''}`}>
      ...
      {!isEmbedded && <Topbar showLogo={true} />}
