...
export async function detectProfileFromText(text, { useLlm = false } = {}) {
  const res = await fetch(`${API_BASE}/api/profiles/detect`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      text: String(text || ''),
  ...
  if (!res.ok) await throwApiError(res, 'Failed to run profile NLP detection')
  return res.json()
}
