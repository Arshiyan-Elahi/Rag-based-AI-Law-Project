...
function markdownToHtml(md) {
  const source = typeof md === 'string' ? md : String(md || '')
  if (!source) return ''
  let html = source
    // headings
    .replace(/^### (.+)$/gm, '<h3>$1</h3>')
...
export default function ProfileWorkspacePage() {
  ...
  const [nlpDraft, setNlpDraft] = useState({
    loading: false,
    lastTextHash: '',
    summary: '',
    detectedDomain: '',
  ...
  const loadProfiles = useCallback(async () => {
    ...
    try {
      ...
      if (normalizedRows.length > 0) {
        setSelectedProfileId((prev) => prev || String(normalizedRows[0].id))
      }
    } catch (err) {
    ...
    try {
      ...
      if (!md) {
        try {
          ...
          md = typeof mdResp?.markdown === 'string' ? mdResp.markdown : String(mdResp?.markdown || '')
        } catch {
          // ignore
        }
      }

      setProfileMd(String(md || ''))
      setEditorValue(String(md || ''))
      setNlpDraft((prev) => ({
        ...prev,
        lastTextHash: hashText(md),
        summary: '',
        detectedDomain: '',
        confidence: 0,
        entities: [],
  ...
  useEffect(() => {
    ...
    const text = String(editorValue || '').trim()
    if (!text || text.length < 80 || !selectedProfileId) return undefined
    const textHash = hashText(text)
    if (textHash && textHash === nlpDraft.lastTextHash) return undefined

    const timer = setTimeout(async () => {
      setNlpDraft((prev) => ({ ...prev, loading: true, error: '' }))
      try {
        const detected = await detectProfileFromText(text, { useLlm: false })
        setNlpDraft({
          loading: false,
          lastTextHash: textHash,
          summary: String(detected?.summary || ''),
          detectedDomain: String(detected?.detected_domain || ''),
          confidence: Number(detected?.overall_confidence_score || 0),
          entities: Array.isArray(detected?.suggestions)
            ? detected.suggestions.map((s) => String(s?.suggestion_type || 'general')).filter(Boolean).slice(0, 20)
            : [],
          error: '',
        })
      } catch (err) {
        setNlpDraft((prev) => ({
          ...prev,
          loading: false,
          error: err?.message || 'Profile NLP detection failed.',
        }))
      }
    }, 700)

    return () => clearTimeout(timer)
  }, [viewMode, editorValue, selectedProfileId, nlpDraft.lastTextHash])
  ...
  const pj = asObject(selectedProfile?.active_profile_json)
  const detectedDomains = asArray(pj.detected_domains).join(', ') || 'N/A'
  const detectedDepts = asArray(pj.detected_departments).join(', ') || 'N/A'
  const sopTypes = asArray(pj.detected_sop_types).join(', ') || 'N/A'
  const preferredStyle = asObject(pj.preferred_style)
  const formality = preferredStyle.formality || 'N/A'
  const risksGaps = asObject(pj.risks_gaps)
  const riskScore = asObject(risksGaps.risk_score)
  const riskLevel = String(riskScore.level || 'unknown').toLowerCase()
  const gapCount = Number(risksGaps.gap_count ?? 0) || 0
  const rolesRaci = asObject(pj.roles_raci)
  const rolesDict = asObject(rolesRaci.roles && typeof rolesRaci.roles === 'object' ? rolesRaci.roles : rolesRaci)
  const rolesCount = Object.values(rolesDict).filter(v => typeof v === 'object' && v?.detected).length
  const complianceElements = asObject(pj.compliance_elements)
  const complianceStandardsList = asArray(complianceElements.standards_detected)
  const complianceStandards = complianceStandardsList
          ...
          {viewMode === 'edit' && (
            <div className="profile-editor-meta">
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: '#6b7280' }}>
                {nlpDraft.loading ? 'NLP analyzing draft…' : `NLP domain: ${nlpDraft.detectedDomain || 'N/A'} | confidence: ${(Number(nlpDraft.confidence || 0) * 100).toFixed(0)}%`}
              </div>
              {nlpDraft.summary ? (
              ...
              {nlpDraft.entities?.length ? (
                <div style={{ marginTop: 6, fontSize: 12, color: '#4b5563' }}>
                  Entities: {nlpDraft.entities.join(', ')}
                </div>
              ) : null}
              {nlpDraft.error ? (
                <div style={{ marginTop: 6, fontSize: 12, color: '#b91c1c' }}>
                  NLP Error: {nlpDraft.error}
                </div>
              ) : null}
            </div>
          )}
          ...
          {selectedProfile ? (
            <div className="profile-params-body">
              ...
              <div className="profile-param-section">
                ...
                {complianceStandardsList.map((s, i) => {
              ...
              <div className="profile-param-section">
                ...
                {[
                  ...asArray(asObject(pj.terminology).acronyms).slice(0, 5).map(t => ({ t, type: 'acronym' })),
                  ...asArray(asObject(pj.terminology).controlled_terms).slice(0, 5).map(t => ({ t, type: 'controlled' })),
                ].map(({ t, type }, i) => (
                  <div key={i} className="profile-param-item profile-param-item--purple">
                    {t} <span className="profile-param-sub">({type})</span>
                  </div>
                ))}
                {!asArray(asObject(pj.terminology).acronyms).length && !asArray(asObject(pj.terminology).controlled_terms).length && (
              ...
              <div className="profile-param-section">
                ...
                {asArray(risksGaps.critical_focus_areas).map((area, i) => (
                  <div key={i} className="profile-param-item profile-param-item--amber">⚠ {area}</div>
                ))}
                {!asArray(risksGaps.critical_focus_areas).length && (
              ...
              <div className="profile-param-section">
                ...
                {Object.entries(asObject(pj.workflow_patterns))
                  .filter(([, v]) => v?.detected)
                  .map(([wf]) => (
                    <div key={wf} className="profile-param-item profile-param-item--green">✓ {wf}</div>
                  ))}
                {!Object.values(asObject(pj.workflow_patterns)).some(v => v?.detected) && (
                  <div className="profile-param-empty">None detected</div>
                )}
              </div>
