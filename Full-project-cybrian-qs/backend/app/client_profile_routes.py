...
from .services.profile_analysis import analyze_sop_text_profile

logger = logging.getLogger(__name__)

router = APIRouter()

# Local numeric guard for NLP confidence values.
...
def create_manual_profile_version(
    ...
    merged_json["manual_edit_reason"] = payload.change_reason or "manual_edit"

    if payload.run_nlp_refresh:
        try:
            detected = analyze_sop_text_profile(
                payload.profile_md,
                use_llm=False,
            ) or {}
            prev_hist = merged_json.get("nlp_session_history")
            if not isinstance(prev_hist, list):
                prev_hist = []
            session_row = {
                "summary": detected.get("summary") or "",
                "detected_domain": detected.get("detected_domain") or "",
                "overall_confidence_score": _safe_confidence(
                    detected.get("overall_confidence_score"),
                    default=0.0,
                ),
                "entities": [
                    str((s or {}).get("suggestion_type") or "general")
                    for s in (detected.get("profile_suggestions") or [])
                    if isinstance(s, dict)
                ][:100],
                "routing_intent": (
                    "style_update"
                    if (detected.get("profile_suggestions") or [])
                    else "profile_update"
                ),
            }
            merged_json["latest_nlp_session"] = session_row
            merged_json["nlp_session_history"] = (prev_hist + [session_row])[-10:]
        except Exception as exc:
            logger.exception("[profile] manual save NLP refresh failed profile_id=%s", profile_id)
            raise HTTPException(
                status_code=500,
                detail=f"Profile saved aborted: NLP refresh failed ({exc})",
            )
    ...
    db.add(
        ProfileHistoryEvent(
            client_profile_id=profile.id,
            event_type="manual_profile_saved",
            event_summary=(payload.change_reason or "Manual profile edit from workspace"),
            metadata_json={
                "run_nlp_refresh": bool(payload.run_nlp_refresh),
            },
        )
    )
