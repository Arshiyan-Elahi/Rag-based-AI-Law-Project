...
def structure_lines_to_blocks(lines: List[str]) -> List[Dict[str, Any]]:
    ...
    def flush_paragraph() -> None:
        if not para_buffer:
            return
        if len(para_buffer) == 1:
            text = _clean_line(para_buffer[0])
        else:
            text = "\n".join(_clean_line(line) for line in para_buffer if _clean_line(line))
        if text:
            from ..utils.table_blocks import paragraph_text_looks_like_table, table_block_from_paragraph_text
