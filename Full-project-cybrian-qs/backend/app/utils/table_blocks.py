"""
Normalize extracted table grids for TipTap / ProseMirror (rectangular rows, header hints).
"""
...
def normalize_table_rows(rows: List[List[Any]]) -> List[List[str]]:
    """Pad/truncate rows to a uniform column count; drop fully empty rows."""
...
def infer_header_row_count(rows: List[List[str]], explicit: int | None = None) -> int:
    """
    Guess how many leading rows are column headers.
    Prefer explicit extractor hint; otherwise use lightweight heuristics.
    """
    if not rows:
        return 0
    ...
    def _row_stats(row: List[str]) -> tuple[float, int]:
    ...
    avg0, labels0 = _row_stats(rows[0])
    avg1, labels1 = _row_stats(rows[1])

    # Two short label rows (common in SOP change-history tables)
    if len(rows) >= 3 and avg0 < 42 and avg1 < 42:
        avg2, _ = _row_stats(rows[2])
...
def table_block_from_rows(
    ...
    *,
    ...
    if not normalized:
        return None
    count = infer_header_row_count(normalized, header_rows)
    return {
        "type": "table",
        "rows": normalized,
        "header_rows": count,
...
def paragraph_text_looks_like_table(text: str) -> bool:
    """Detect plain-text tables (tabs / multi-space columns) mistaken as paragraphs."""
