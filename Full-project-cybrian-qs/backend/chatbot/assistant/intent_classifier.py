...
B) flow = "editor_action" — user wants the **open SOP text** changed or generated in-place:
   ...
   - Summarize requests that ask to transform/shorten the open SOP content are editor actions.
    ...
    *,
    ...
    if flow == "chat":
        ...
        if not has_active_sop:
            ...
            flow = "clarify"  # type: ignore[assignment]
            clarification = "I detected an editor action request, but the action type is missing. Please specify rewrite, improve, gap_check, summarize, or analyze."
        else:
    ...
    *,
    ...
    try:
        ...
        return AssistantIntentResult(
            flow="clarify",
            confidence=0.0,
            reasoning="llm_classifier_error",
            clarification_question=(
                "I could not classify this reliably. Do you want a chat answer or an editor action "
                "(rewrite, improve, gap_check, summarize, analyze)?"
            ),
        )
